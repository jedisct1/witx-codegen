...
",
    ...
    fn define_as_enum<T: Write>(
        ...
        ))?;
        {
            ...
            for (i, variant) in enum_data_type.variants.iter().enumerate() {
                ...
                w.write_line(format!("{} = {},", variant.name.as_str().to_uppercase(), i))?;
            }
        }
        w.write_line("};")?.eob()?;
        Ok(())
    ...
    fn define_as_int<T: Write>(
        ...
        w.write_line(format!("pub const {}_values = struct {{", as_type))?;
        {
            let mut w = w.new_block();
    ...
    fn define_as_flags<T: Write>(
        ...
        w.write_line(format!("pub const {} = {};", as_type, actual_as_type))?;
        w.write_line(format!("pub const {}_bits = struct {{", as_type))?;
        {
            let mut w = w.new_block();
    ...
    fn define_union_variant_accessors<T: Write>(
        ...
        match variant.tref.as_ref() {
            None => {
                w.write_line(format!("pub fn {}() {} {{", variant_name, as_type))?
                    .indent()?
                    .write_line(format!("return {}.new({});", as_type, i))?
                    .write_line("}")?
                    .eob()?;

                w.write_line(format!("pub fn set_{}(self: *Self) void {{", variant_name))?
                    .indent()?
                    .write_line(format!("self.tag = {};", i))?
                    .write_line("}")?
                    .eob()?;

                w.write_line(format!("pub fn is_{}(self: Self) bool {{", variant_name))?
                    .indent()?
                    .write_line(format!("return self.tag == {};", i))?
                    .write_line("}")?;
            }
            Some(variant_type) => {
                ...
                w.write_line(format!(
                    "pub fn set_{}(self: *Self, val: {}) void {{",
                    variant_name, as_variant_type
                ))?;
                {
                    w.new_block()
                        .write_line(format!("self.tag = {};", i))?
                        .write_line("self.set(val);")?;
                }
                w.write_line("}")?.eob()?;
                ...
                }
                {
                    let mut w = w.new_block();
                    if as_variant_type.is_nullable() {
                        w.write_line(format!("if (self.tag !== {}) {{ return null; }}", i))?;
                    }
                    w.write_line(format!("return self.get({});", as_variant_type))?;
                }
                w.write_line("}")?;
            }
        }
    ...
    fn define_as_union<T: Write>(
        ...
        w.write_line(format!("pub const {} = struct {{", as_type))?;
        {
            ...
            for i in 0..pad_len / 8 {
                w.write_line(format!("__pad64_{}: u64 = 0,", i))?;
            }
            for i in 0..(pad_len & 7) / 4 {
                w.write_line(format!("__pad32_{}: u32 = 0,", i))?;
            }
            for i in 0..(pad_len & 3) / 2 {
                w.write_line(format!("__pad16_{}: u16 = 0,", i))?;
            }
            for i in 0..(pad_len & 1) {
                w.write_line(format!("__pad8_{}: u8 = 0,", i))?;
            }
            w.eob()?;
            w.write_line("const Self = @This();")?;
            w.eob()?;
            w.write_line(format!("pub fn new(tag: {}) void {{", as_tag))?;
            {
                let mut w = w.new_block();
                w.write_line("return Self { .tag = tag };")?;
            }
            w.write_line("}")?.eob()?;

            w.write_line(format!(
                "pub fn init(comptime T: type, tag: u8, val: T) {} {{",
                as_type
            ))?;
            {
                let mut w = w.new_block();
                w.write_line(format!("var tu = {}.new(tag);", as_type))?
                    .write_line("tu.set(val);")?
                    .write_line("return tu;")?;
            }
            w.write_line("}")?.eob()?;

            w.write_line("pub fn get(self: *const Self, comptime T: type) T {")?;
            {
                let mut w = w.new_block();
                w.write_line(format!("const valBuf = @ptrToInt(self) + {};", val_offset))?;
                w.write_line("return @intToPtr(* const T, valBuf).*;")?;
            }
            w.write_line("}")?.eob()?;

            w.write_line("pub fn set(self: *Self, comptime T: type, val: T) void {")?;
            {
                let mut w = w.new_block();
                w.write_line(format!("var valBuf = @ptrToInt(self) + {};", val_offset))?
                    .write_line(format!("mem.set(@intToPtr([{}]u8, valBuf), 0);", val_size))?;
                w.write_line("@intToPtr(*T, valBuf).* = val;")?;
            }
            w.write_line("}")?;

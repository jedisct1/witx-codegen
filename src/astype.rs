...
impl fmt::Display for ASType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ASType::Ptr(other_type) => match other_type.as_ref() {
                ASType::Char | ASType::U8 => write!(f, "[*] const {}", other_type),
                _ => write!(f, "* const {}", other_type),
            },
            ASType::MutPtr(other_type) => match other_type.as_ref() {
                ASType::Char | ASType::U8 => write!(f, "[*]{}", other_type),
                _ => write!(f, "* {}", other_type),
            },
            ASType::Struct(None) => write!(f, "usize /* struct */"),
            ASType::Struct(Some(name)) => write!(f, "struct_{}", name),
            ASType::WasiStringPtr => write!(f, "[*] const char"),
            ASType::Handle => write!(f, "handle"),
            ASType::WasiString => write!(f, "[*] u8"),
            ASType::Union(None) => write!(f, "usize /* union */"),
            ASType::Union(Some(name)) => write!(f, "union_{}", name),
